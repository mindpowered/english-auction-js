...
class englishauction_EnglishAuction {
	...
	registerMyMethods() {
		...
		this.maglev.register("EnglishAuction.GetStart",function(args) {
			let id = args[0];
			let auction = bus.call("Persistence.Get",["Auction","FindById",[id]]);
			let start = 0;
		...
		this.maglev.register("EnglishAuction.GetEnd",function(args) {
			let id = args[0];
			let auction = bus.call("Persistence.Get",["Auction","FindById",[id]]);
			let end = 0;
		...
		this.maglev.register("EnglishAuction.HasStarted",function(args) {
			let id = args[0];
			let auction = bus.call("Persistence.Get",["Auction","FindById",[id]]);
			let start = 0;
			if(typeof(auction.h["start"]) == "string") {
		...
		this.maglev.register("EnglishAuction.HasEnded",function(args) {
			let id = args[0];
			let auction = bus.call("Persistence.Get",["Auction","FindById",[id]]);
			let end = 0;
			if(typeof(auction.h["end"]) == "string") {
