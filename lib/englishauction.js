...
class englishauction_EnglishAuction {
	...
	registerMyMethods() {
		...
		this.maglev.register("EnglishAuction.GetStart",maglev_MagLevFunction.fromFunction(function(args) {
			let id = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let arr = ["EnglishAuction.Auction","FindById",[id]];
			let auction = _gthis.convertToHaxe(bus.call("Persistence.Get",js_Boot.__cast(_gthis.convertToMagLev(arr) , maglev_MagLevArray)).getResult());
			let start = 0;
			if(typeof(auction.h["start"]) == "string") {
				start = HxOverrides.strDate(auction.h["start"]).getTime();
			} else {
				start = auction.h["start"];
			}
			let result = start;
		...
		this.maglev.register("EnglishAuction.GetEnd",maglev_MagLevFunction.fromFunction(function(args) {
			...
			let auction = _gthis.convertToHaxe(bus.call("Persistence.Get",js_Boot.__cast(_gthis.convertToMagLev(arr) , maglev_MagLevArray)).getResult());
			let end = 0;
			if(typeof(auction.h["end"]) == "string") {
				end = HxOverrides.strDate(auction.h["end"]).getTime();
			} else {
				end = auction.h["end"];
			}
			let result = end;
	...
	convertToHaxe(x) {
		if(x.getType() == maglev_MagLevNull.getStaticType()) {
			...
			let arr = [];
			let i = 0;
			while(i < y.size()) arr.push(y.get(i));
			return arr;
